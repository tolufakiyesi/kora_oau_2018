...
class User_model extends CI_Model {
    ...
    public function authenticate_user($user) {
        ...
        //user detail is being updated
        if(isset($user['id']) && $this->get_user($user['id'])){
            if ($this->super_update($user['id'], $user)){
                return $this->get_user($user['id']);
            }
            return false;
        }
        if($dbuser = $this->get_user_by_detail('email', $user['email'])){
            if ($this->super_update($dbuser->id, $user)){
                return $this->get_user($dbuser->id);
            }
            return false;
        }
    ...
    public function get_user_by_detail($key, $val) {
    ...
    public function send_password_reset_link($user_detail){
        ...
        $user = $this->get_user_by_detail('email', $user_detail);
    ...
    public function validate_user()
    {
        ...
        $cookie = $this->input->get_request_header('Session-Cookie', TRUE);


        if ($this->verify_cookie($user_id, $cookie)){
            return true;
        }
    ...
    public function verify_cookie($user_id, $cookie_key){
        $this->db->from('users');
        $this->db->where('id', $user_id);
        $this->db->where('session_cookie', $cookie_key);
        return $this->db->get()->row();
    }
    ...
    private function send_confirmation_email($email) {
        ...
        $user = $this->get_user_by_detail('email', $email);
